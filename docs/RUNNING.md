...
| flash | `CAMR.ps_flash_tau = 1e-7` s (default; nucleation from an absent phase and the saturation-projection birth are always on) | `CAMR.ps_flash_tau = 0` (off) |
...
| `ps_wp_transverse` | 0 | Transverse coupling in 2-D/3-D: 0 off (directionally split), 1 contact-only LeVeque transverse term. 2 (the former acoustic term) is retired and aborts. | production (0); 1 alternative |
...
| `ps_flash_tau` | 1e-7 s | Flash (nucleation) source time scale; ≤ 0 = off. Nucleation from an ABSENT phase (birth at $\alpha_{\mathrm{birth}}$) and placement of the newborn at its saturation density are always on. | production |
...
| `ps_mt_tau_model` | 0 | 0 constant τ; 2 τ derived from the SRT rate (Lund & Aursand). 1 (the HRM correlation) is retired and aborts. | alternative |
...
| `ps_bc_use_nscbc` | 0 | Global characteristic (NSCBC) outflow on `Inflow`-typed faces; 0 = `bcnormal` interior-copy/reservoir path. The ghost construction is always the choked fan with HEM flash along the fan. Force-added. | production |
...
| `ps_pres_diag` | 0 | `[PS-X3]`, `[PS-DT]`, `[PS-GATE]` per-sweep outcome tables of the coupled source, and the `[PS-FLASH-REF]` flash-refusal table. | diagnostic (kept) |
...
| `ps_mt_diag` | 0 | Split-source transfer statistics and the `[PS-MTCAUSE]` refusal table; ≥ 2 adds a contraction-ratio re-solve per cell. | diagnostic |
...
| `eos_table` | 1 | PRTab/GERGTab: use the table on the hot paths; 0 = analytic. The former alias `eos_mlp` is retired and aborts. | production (table builds) |
| `eos_table_auto` | 1 | Table on the auto-detect (mixture) path too; 0 forces analytic there. The former alias `eos_mlp_auto` is retired and aborts. | alternative |
...
Retired keys abort merely by being present, so a harness that swallows stderr hides the abort; run a new deck once by hand. The single table of retired keys is `Source/Hydro/PelantiShyue/PS_retired_keys.H`, checked once at startup. Retired keys, grouped by what replaced them: `ps_flux` with any value but `wp`; the deleted flux-path selectors `ps_recon`, `ps_alpha_limiter`, `ps_pk_energy_flux`, `ps_ctu`, `ps_star_relaxed` (the relaxed-α star state is the only path), `ps_rk_model`, `ps_llf_identity` (the identity-consistent LLF fallback is the only fallback), `ps_wp_proj_scale` (the nondimensional projection is the only projection), `ps_shear_diss` (the Jameson shear dissipation is deleted; use `ps_mu`); the deleted presence selectors `ps_alpha_vanish` (the live knob is `ps_presence_vanish`), `ps_presence`, `ps_cmix_model`, `ps_relax_hyst` (the relaxation-gate hysteresis is always on in host builds; the GPU build never had it), `ps_resync_mass` (the mixture-mass resync always runs), `ps_dilute_closure`, `ps_dilute_alpha0`; the deleted source selectors `ps_theta_model`, `ps_hrm_theta`, `ps_mt_target` (the step-consistent Lund–Aursand target is the only target), `ps_src_p_reproject` (sources always re-establish $P_1 = P_2$ in the cells they changed), `ps_flash_from_absent`, `ps_flash_project_sat` (nucleation from an absent phase at the saturation density is the only birth); the deleted `ps_state_from_cons` switches `ps_p_mode`, `ps_p_clip`, `ps_c_mode`, `ps_y_floor` (volume-weighted $P_{\mathrm{mix}}$ and the Wood frozen sound speed are the only branch); the deleted boundary selectors `ps_bc_nscbc_v2`, `ps_bc_nscbc_flash` (the flash-aware choked fan is the only ghost construction); the deleted EOS hooks `eos_warmstart`, `eos_warmstart_fixed`, `eos_mlp`, `eos_mlp_auto`; the deleted state harvester `ps_harvest`, `ps_harvest_trace`, `ps_harvest_afloor`, `ps_harvest_dlr`, `ps_harvest_de`, `ps_harvest_file`, `ps_harvest_flush`; and the deleted investigation diagnostics and probes `ps_prehydro_diag`, `ps_psat_diag`, `ps_t2_diag`, `ps_diag_morph`, `ps_eovs_diag`, `ps_coexit_diag`, `ps_prdiag`, `ps_relax_diag`, `ps_floor_budget`, `ps_dm_census`, `ps_relax_verbose`, `ps_flash_ev_diag`, `ps_dilute_probe`, `ps_m2_test`, `ps_asy1_probe`. Retired values of live keys, which also abort: `ps_wp_transverse = 2`, `ps_coexist_action = 2/3/4`, `ps_relax_mode = 3`, `ps_mt_tau_model = 1`.
...
together with `[PS-GATE]` (presence-gate refusals, all modes) and `[PS-FLASH-REF]`, the flash kernel's refusal table by cause. A line ending `*** EVERY cell structurally dead: the operator is a NO-OP this sweep` means the source did nothing. The isochoric pressure-relaxation Newton statistics of modes 1/2 (`PsPrStats`) are counted but not reported. Independently of these, `CAMR.v=1` prints the cumulative counters `[ps_flash] cumulative cells flashed = N` and `[ps_mt] cumulative cells with finite-rate MT = N` (split source, modes ≠ 5) whenever they advance, and the cause tables `[PS-FLCAUSE]`, `[PS-RELAXFB]`, `[PS-PROMOTE]`, `[PS-FOLD]`, `[PS-MTCAUSE]` at the mass-diagnostic cadence. A healthy two-phase 2-D run shows `[ps_mt]` advancing, `[PS-PROMOTE]` refusals at zero or decaying, and `[PS-FOLD]` counts bounded.
...
Every plotfile and checkpoint carries a `job_info` file with the full ParmParse table (`Utils/CAMR_io.cpp`). Dials whose resolved value matters even when the deck is silent are force-added to that table at their read site: `ps_flux`, `ps_do_relax`, `ps_strang`, `ps_bc_use_nscbc`, `ps_bc_nscbc_sigma`, `ps_bc_nscbc_order`, the six per-face NSCBC enables and six per-face ambient targets after inheritance, `gerg_ext_c`, `lazy_temp`, `ppm_trace_sources`. `job_info` is therefore the record of what a plotfile was made with; a key that does not appear there was not read. An exact 1.000 rel-L2 in a comparison means an absent field, not bad physics.
...
- `Exec/fingerprint_plt.py <plt>`: level-0 fingerprint of a 2-D plotfile, per variable the SHA of the field bytes plus min, max and mean; change attribution for the 2-D decks. The recorded fingerprints live in `Exec/CO2_TBlowdown/refs/` and `Exec/CO2_B4/refs/` (`fingerprint_BASE.txt`).
- `Exec/CO2_B4/score_b4.py <plt> [...]`: rel-L2 (ρ, u, P) of a B4 deck's plotfile against the frozen exact B4 profile (the vendored `CO2_RiemannSuite/refs/exact/` set); the reference numbers are in `Exec/CO2_B4/refs/` and `VERIFICATION.md` §7.4.
