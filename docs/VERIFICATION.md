...
### 7.6 Mode-5 evidence on the pipe-break (`Exec/CO2_PipeBreak/mode5_evidence.sh`)

The evidence that qualified the coupled X3 operator (`ps_relax_mode = 5`) on the production deck, and the reason mode 2 is still in the tree ([DECIDE-3], `DESIGN_DECISIONS.md` F-5). `mode5_evidence.sh {demo2|demo2-mode5|windows|demo3|sym|report}` runs each stage on 6 ranks (the demo2 mode-5 stage is checkpointed and resumable) and `report` prints the final step, abort count, the `[PS-GUARD]/[PS-RELAXFB]/[PS-PROMOTE]` totals and the per-stage `ps_validate` census; `x3_cost.sh` builds the TinyProfiler executable and runs demo3 to step 50 in both modes with `ps_pres_diag = 1` for the operator cost. Rule 3 applies: completion is the entry condition, the census and the mode-2 comparison are the measurement.

Second pass, code at `99a067e` (Newton predictor and line search R-13, physical trace seed R-15, α-only Corridor projection R-14, additive resync G-5), 2026-09-10. All eight runs complete with zero aborts.

| stage | mode 2 (control) | mode 5 | reading |
|:--|:--|:--|:--|
| demo2 to 2.5 ms | 567 steps, 3.3 h | 648 steps, 3.2 h (18 s per coarse step; 100–700 s before R-13) | completes; cost parity |
| demo2 matched-time liquid inventory (`compare_pair.py`) | — | +6.3 to +8.3 % (net condensation, a shell at the jet head at 43–44 bar); roughness ratio 1.2–1.35 | a model difference, open as `FUTURE_WORK.md` §6.2 |
| `sym`: mirror asymmetry at step 50, `max_level = 0` | 5e-10 | 0.113 | round-off seed at step 2 in both modes, amplified ×~1.4/step by mode 5 from step ~12; the rung is not a mode-5 acceptance (`FUTURE_WORK.md` §6.3) |
| demo3 step 50, contact window $\max\lvert d^2\alpha_1\rvert$ / extrema | 1.16e-3 / 3 | 1.20e-3 / 4 | pass; the front window (7.2e-2 vs 1.2e-3) is the condensation bump |
| window 3550→3605 | reference heals `[PS-PROMOTE]` refusals by 3684 | refusals 0 throughout; split amplification 6.7 | cleaner than the reference |
| window 3650→3700 | 235/step decaying to 0 by 3684 | 0 until 3670, then rising to 118 by 3698; 26 Independent liquids with no root at the end | worse: a bulk liquid expanded past its spinodal without evaporating, `FUTURE_WORK.md` §6.1 |

Operator cost (`x3_cost.sh`, demo3 to step 50, before R-13): run time 90.6 s (mode 2) against 886.7 s (mode 5); `ps_apply_relaxation` exclusive 9.8/17.8/26.4 s against 110/341/489 s over ranks; X3 per call 1.28 sub-steps, 13.8 Newton iterations, 39.8 path evaluations, 14 % of sub-steps non-converged. After R-13 the demo2 deck runs at cost parity; the 2-D non-convergence fraction (~15 %) is unchanged and is an open derivation.

First pass (code before those four changes) is recorded for attribution: demo2 mode 5 aborted at step 608 on a Corridor cell whose vapour host the hydro had made unevaluable (the multiplicative resync, G-5), the mirror rung failed at 0.72 with `max_level = 0`, and the deck ran 10–20× slower than mode 2. Each of those had one cause and each cause is a landed decision, so the first-pass numbers are not references.

The 2-D references (`regen_refs.sh 2d BASE`, `regen_refs.sh windows BASE`) are re-recorded at the retirement commit, since the physical seed and the Corridor projection change every trace phase in the pipe-break; the PRE windows were never recorded (no MPI in the workspace where phase 0 ran), so BASE is the first 2-D fingerprint set. It is recorded in mode 2 as the decks ship.
