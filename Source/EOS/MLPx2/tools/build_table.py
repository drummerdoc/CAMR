"""Generate the MLPx2 bicubic PR-EOS table (params header + data TU).

Self-contained: compiles gen_table.cpp against CAMR's own RealFluidCO2 PR
...
(auto-detect, liquid, vapor), fills the small invalid corner, and writes
    ...
    ../mlpx2_table_params.H   -- grid constants + norm (tiny)
    ../mlpx2_table_data.cpp   -- the six bicubic table arrays (~6 MB)

Both outputs are GIT-IGNORED; the committed mlpx2_fwd_net.H includes the
...
MLPX2 = os.path.normpath(os.path.join(HERE, ".."))
RF = os.path.normpath(os.path.join(MLPX2, "..", "RealFluidCO2"))
...
def main():
    ...
    ap.add_argument("--params", default=os.path.join(MLPX2, "mlpx2_table_params.H"))
    ap.add_argument("--data", default=os.path.join(MLPX2, "mlpx2_table_data.cpp"))
    a = ap.parse_args()

    # 1. compile gen_table.cpp against CAMR RealFluidCO2 PR
    ...
    # ---- vectorized Catmull-Rom base bicubic (mirrors mlpx2_bicubic) ----
    ...
    p = ("#ifndef MLPX2_TABLE_PARAMS_H\n#define MLPX2_TABLE_PARAMS_H\n"
