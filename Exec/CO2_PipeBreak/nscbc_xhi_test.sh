...
declare -A OPT=( [A]="" [B]="CAMR.ps_bc_nscbc_xhi=0" [C]="CAMR.ps_bc_nscbc_sigma=1.0" [D]="CAMR.ps_bc_nscbc_sigma=4.0" [E]="CAMR.ps_bc_nscbc_order=1" )
for V in $VARS; do
  d=runs/nscbc_xhi/$V; mkdir -p "$d"
  echo "== variant $V: ${OPT[$V]:-as shipped}  -> $d"
  ( cd "$d" && mpiexec -n "$NR" "../../../$EXE" ../../../inputs.satjet_demo3 "${COMMON[@]}" amr.plot_file=plt_${V}_ amr.check_file=chk_${V}_ ${OPT[$V]} > run.log 2>&1 ) || echo "   variant $V ended with an error — see $d/run.log"
done
