...
| Deck / script | Purpose |
|---|---|
| `inputs.satjet_demo2` | Reference production deck: wp, `ps_wp_order = 2`, `ps_bl_reflux = 2` (the historical spelling of on: any nonzero value is the α co-move at reflux), `ps_relax_mode = 2` with `ps_theta_tau = ps_mt_tau = 1e-3` s, flash off, `ps_pres_floor = 1e5` Pa, `ps_temp_floor = 216.6` K, `ps_mu = 2` (the deleted `ps_shear_diss` key is no longer set), α tag 0.1, `n_error_buf = 2`; output prefixes `plt_sj2_`/`chk_sj2_`. Its relaxation configuration differs from the 1-D defaults (mode 5, τ = 1e-7): see `docs/RUNNING.md`. |
