...
| `verify_canonical.py` | The gate (~30 s, no arguments), checks 0–6: executable newer than every `*.H`/`*.cpp` under `Source/`; frozen A/C battery rel-L2(ρ,u,P) per case ±2e-3 with mean 0.0350 and C1 exactly 0.000, at `alpha_trace = 1e-6` and at `alpha_trace = 0`; B4 flatness (u-error 0.129 ± 0.02 at τ = 1e-4 and 1e-7, Δ < 0.01, at the default flash margin 0.10); reproject liveness on B9 (0.845 vs 0.752); frozen B4 at zero trace 0.131; frozen B5 0.388 and B2 front stable with the pelanti kernel; B12 two-phase wall reflection with far field undisturbed (< 1e-6 P₀), mirror symmetry < 1e-6 and reflection ratio $c_1/c_2 \le 0.25$ (measured 0.0097). |
...
| `cleanup_session.sh` | Removes the harness run directories (`vc_ vc4_ vcrp0_ vcrp1_ vp_ vz_ v4_ vcb12_ ex_wp_ ch_ fs_ hem_ cvg* conv_data std_ref flash_red_runs`, `*.old.*`) and scratch logs; touches nothing tracked. |
| `refs/`, `characterization/`, `fig4_digitized_curves.csv`, `family_reproduction.png` | Vendored exact/HEM profiles with provenance and the recorded gate transcripts; fingerprint records; the DT7 digitisations. |
